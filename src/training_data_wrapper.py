...
import util
from datasets import GenotypeDataset, UKBGeneLevelDataset
...
class TrainingDataWrapper:
    ...
    def __init__(self, args):
        ...
        self.input_features = np.dstack(
            [self.mutations, self.cn_deletions, self.cn_amplifications]
        )

        self.train_feature, self.train_label, self.val_feature, self.val_label = (
            self.prepare_train_data()
        )

    def prepare_train_data(self):
        return util.prepare_train_data(
            self.train, self.cell_id_mapping, self.zscore_method, self.std
        )
...
class TrainingDataWrapperGeno:
    ...
    def prepare_train_data(self):
        return util.prepare_train_data(
            self.train, self.sample_id_mapping, self.zscore_method, self.std
        )
...
class TrainingDataWrapperGenoUKB:
    ...
    def __init__(self, args):
        ...
        # print number of genes and number of samples
        print("Number of genes (input features): ", len(self.dataset.gene_id_mapping))
        print("Number of samples: ", len(self.dataset.labels))
        print("Shape of dataset: ", self.dataset.data.shape)
    ...
    def create_dataset(self, args):
        # load the data
        self.dataset = UKBGeneLevelDataset(args)
        self.gene_id_mapping = self.dataset.gene_id_mapping
