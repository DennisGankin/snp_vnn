...
def main():
    ...
    pytorch_total_params = sum(p.numel() for p in go_vnn_model.parameters() if p.requires_grad)
    ...
    train_dataset, val_dataset = torch.utils.data.random_split(data_wrapper.dataset, [0.8, 0.2])
    # create dataloaders
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batchsize, shuffle=True, num_workers=31)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.batchsize, shuffle=False, num_workers=31)
    ...
    wandb_logger = WandbLogger(project='vnn_ukb_breast_cancer', name='nest_reported_'+curr_time)
    logger = TensorBoardLogger(name="nest_ukb_model_"+curr_time, save_dir="/home/dnanexus/lightning_logs")
    #trainer = L.Trainer(max_epochs=args.epoch, logger=logger)
    trainer = L.Trainer(max_epochs=args.epoch, 
                        logger=[logger, wandb_logger],
                        log_every_n_steps=2,
                        callbacks=[lr_monitor]) # log every steps should depend on the batch size
    ...
    # Train the model
    trainer.fit(model=go_vnn_model, train_dataloaders=train_loader, val_dataloaders=val_loader)
    # Record the end time
    end_time = time.time()
